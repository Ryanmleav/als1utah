import React from "react";
import Header from "./Header";

const Schedule = () => {
  return (
    <div className="Home">
      <Header />
      <div>
        <h2 class="trainsched">ALS1UTAH Training Schedule</h2>
      </div>
      {/* <CalendarComponent id="calendar" /> */}
      <iframe
        className="calander"
        src="https://calendar.google.com/calendar/embed?src=hopelessgains%40gmail.com&ctz=America%2FDenver"
        title="classschedule"
        // style="border: 0"
        // width="800"
        // height="600"
        // frameborder="0"
        // scrolling="no"
      ></iframe>
      <div className="paymentinfo">
        <img
          alt="googlecalander"
          src="data:image/png;base64,"
        ></img>
        <p className="refunds">
          Please venmo with the class and day you are signing up for. <br></br>
          <br></br>No refunds will be issued on cancellations. You may only
          reschedule one time for another course at a later date for no charge.
          After the first time you reschedule you will be charged a $25 fee to
          reschedule.
        </p>
      </div>
    </div>
  );
};

export default Schedule;
